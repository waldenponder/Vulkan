 ...
 *
 * This sample shows how to upload a 2D texture to the device and how to display
 * it. In Vulkan this is done using images, views and samplers.
 *
...
public:
    ...
    void screenShot()
    {
        VkImage image = swapChain.images[currentBuffer];
        ...
        VkImage dstImage;
        VK_CHECK_RESULT(vkCreateImage(device, &info, nullptr, &dstImage));
        ...
        allInfo.allocationSize = memReqs.size;
        allInfo.memoryTypeIndex = vulkanDevice->getMemoryType(
            memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

        VkDeviceMemory memory;
        vkAllocateMemory(device, &allInfo, nullptr, &memory);
        vkBindImageMemory(device, dstImage, memory, 0);


        VkCommandBuffer copyBuffer = vulkanDevice->createCommandBuffer(
        ...
        vkCmdPipelineBarrier(copyBuffer, 0,
        ...
        vkCmdPipelineBarrier(copyBuffer, 0, VK_PIPELINE_STAGE_TRANSFER_BIT, 0,
                             0, nullptr, 0, nullptr, 1, &barrier);
        VkImageCopy copyReg{};
        copyReg.srcSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
        copyReg.srcSubresource.layerCount = 1;
        copyReg.dstSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
        copyReg.dstSubresource.layerCount = 1;
        copyReg.extent.width = width;
        copyReg.extent.height = height;
        copyReg.extent.depth = 1; 

        vkCmdCopyImage(copyBuffer, image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
    ...
    VkExtent2D getWindowSize(VkPhysicalDevice physicalDevice,
    ...
    */
    ...
    delete[] textureData;
        ...
        sampler.maxLod = (useStaging) ? static_cast<float>(texture.mipLevels) : 0.0f;
    ...
    void OnUpdateUIOverlay(vks::UIOverlay* overlay) override
    {
        ...
        {
            if (overlay->sliderFloat("LOD bias", &uniformData.lodBias, 0.0f,
